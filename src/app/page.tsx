  ...
  export default function Home() {
    ...
    const startRecording = useCallback(async () => {
      ...
      try {
        ...
        setErrorMessage('녹음을 시작하는 중 오류가 발생했습니다. 브라우저가 음성 인식을 지원하는지 확인해주세요.');
      }
    }, [audioStream, requestMicrophonePermission, setErrorMessage, setInterviewState, initializeAudioContext]);
    ...
    const handleConfirm = async () => {
      if (number.length === 11) {
        try {
          ...
          if (isValidNumber) {
            initializeAudioContext(); // AudioContext 초기화
    ...
    return (
      <main className="min-h-screen flex items-center justify-center bg-gray-100">
        <div className="max-w-md w-full bg-white rounded-3xl shadow-xl overflow-hidden p-8">
          ...
          {interviewState === 'idle' && (
            <>
              <p className="text-xl text-center mb-4 text-gray-600">전화번호를 입력 후 확인버튼 눌러주세요.</p>
