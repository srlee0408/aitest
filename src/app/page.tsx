...
import { checkPhoneNumber } from '@/lib/db'
...
export default function Home() {
  ...
  }, [hasMicPermission, requestMicrophonePermission]);
  ...
  const sendAudioToServer = async (text: string) => {
    console.log('GPT에 보내려는 텍스트:', text);
  ...
  const handleConfirm = async () => {
    if (number.length === 11) {
      const isValidNumber = await checkPhoneNumber(number)
      if (isValidNumber) {
        if (hasMicPermission) {
          setInterviewState('interviewing')
          try {
            const { message, threadId } = await startInterview();
            setThreadId(threadId);
            setInterviewMessage(message);
            await speakText(message);
            console.log('Starting recording after AI speech...');
          ...
          setErrorMessage('마이크 권한이 필요합니다. 브라우저 설정에서 권한을 허용한 후 다시 시도해주세.');
        }
      } else {
        alert('등록되지 않은 전화번호입니다.')
      }
    } else {
      alert('올바른 11자리 번호를 입력해주세요.')
    }
  }

  const handleContinueInterview = async (userMessage: string) => {
    try {
  ...
  const checkInterviewEnd = (response: string): boolean => {
    return response.includes("이로써 모든 면접이 끝났습니다. 수고하셨습니다.");
  }
  ...
  const requestFirstQuestion = async () => {
    try {
      ...
      console.log('GPT 첫 번째 응답:', response.data)
      ...
      setErrorMessage('첫 번째 질문을 요청하는 중 오류가 발생했습니다.')
    }
  }
      ...
      <div className="max-w-md w-full bg-white rounded-3xl shadow-xl overflow-hidden p-8">
        ...
        {interviewState === 'interviewing' && (
          <>
            ...
            <p className="text-lg text-center mb-4 text-gray-800">{transcript}</p>
          </>
        )}
        ...
        {!hasMicPermission && (
          <p className="text-yellow-500 text-center mt-4">마이크 권한이 요합니다. 브라우저 설정에서 권한을 허용해주세요.</p>
        )}
        {errorMessage && (
